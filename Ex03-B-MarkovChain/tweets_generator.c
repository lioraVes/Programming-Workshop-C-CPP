//
// Created by liora on 16/08/2022.
//
 ...
 * @param data- the word we want to print.
 */
static void print_node (char *data)
{
  fprintf (stdout, " %s", data);
}

/**
 * This function dynamically allocates place for a new string and copies the
 * original string info to the new one.
 * @param data_ptr - to copy from.
 * @return  a new dynamically allocated cell which is a copy of the original
 * one.
 */
static char *copy_str (char *data_ptr)
{
 ...
 */
static void generate_tweets (int num_to_generate, MarkovChain *markov_chain)
{
                          ...
                          MarkovChain *markov_chain)
{
  ...
  while (fgets (buf, sizeof (buf), fp) != NULL)
  {
    ...
    while ((word != NULL) && (counter != words_to_read))
    {
      ...
      if (prev_node != NULL
          && markov_chain->is_last (prev_node->data) == false)
      {
        if (add_node_to_counter_list (prev_node, word_node->data, markov_chain)
            == false)
        {
          return EXIT_FAILURE;
        }
 ...
 */
static int files (int argc, char *argv[], MarkovChain *markov_chain)
{
...
int main (int argc, char *argv[])
{
  ...
  *new_database = (LinkedList) {NULL, NULL, 0};
  ...
  new_markov_chain->print_func = (Print_fp) print_node;
  new_markov_chain->comp_func = (Cmp_fp) strcmp;
  new_markov_chain->free_data = free;
  new_markov_chain->copy_func = (Copy_fp) copy_str;
  new_markov_chain->is_last = (IsLast) is_last_word;
